...
所有条件的时程由 `browsing.initialPromptSec`（首次弹窗）、`browsing.finalDurationSec`（总上限）、
`browsing.manualExitUnlockSec`（手动退出解锁）与 `browsing.finalPromptForced`（最终弹窗强制结束）显式定义。

### 研究 1A

时程：10 分钟首次弹窗 → 继续则经问卷提示进入第二阶段 → 20 分钟最终弹窗强制结束。
...
| 条件 | 说明 |
|---|---|
| 1B-G1 | 正常色彩组：全程 100% 饱和度（共同基础对照） |
| 1B-G2 | 低饱和度组：全程 30% 饱和度 |

### 研究 2A

时程同 1A。3（反馈类型）×2（继续方式）加一个无助推基线，共 7 组。

| 条件 | 说明 |
|---|---|
| 2A-G1 | 无反馈、轻点继续基线（共同基础对照） |
| 2A-G2 | 时间反馈-滑动继续 |
| 2A-G3 | 条数反馈-滑动继续 |
| 2A-G4 | 组合反馈-滑动继续 |
| 2A-G5 | 时间反馈-长按继续（5 秒） |
...
时程：单阶段，呈现时间内强制观看；到达呈现时间点后降低饱和度并解锁手动退出，
被试可随时退出且退出时点被记录；20 分钟最终弹窗强制结束。
采用 3（呈现时间）×2（饱和度）加一个饱和度不变的基线对照，共 7 组。
编号沿用原有方案，原 100% 饱和度的 G3、G9 已移除，G6 改为基线对照组。

| 条件 | 呈现时间 | 饱和度 | 退出解锁 |
|---|---:|---:|---:|
| 2B-G1 | 5 分钟 | 30% | 5 分钟 |
| 2B-G2 | 5 分钟 | 65% | 5 分钟 |
| 2B-G4 | 10 分钟 | 30% | 10 分钟 |
| 2B-G5 | 10 分钟 | 65% | 10 分钟 |
| 2B-G7 | 15 分钟 | 30% | 15 分钟 |
| 2B-G8 | 15 分钟 | 65% | 15 分钟 |
| 2B-G6 | —（基线对照） | 100%（全程不变） | 10 分钟 |

基线对照组的退出解锁时间取呈现时间的中间水平（10 分钟）。该选择会影响与 5 分钟、15 分钟
两个水平的可比性，正式实施前应在研究设计中明确说明并预注册。

实现文件：`src/config/conditions.js`。

### 共同基础对照的可合并性

1A-G1、1B-G1、2A-G1 共用同一套无反馈、轻点继续、100% 饱和度的两阶段流程（10 分钟首次弹窗、
未解锁前不能手动退出、经问卷提示进入第二阶段），因此可复用同一对照编码。

2B-G6 虽然同样是无反馈、轻点继续、全程 100% 饱和度，但走的是 2B 自己的单阶段流程：
没有 10 分钟弹窗，到解锁时间后可自发退出，测量的是自发退出率与退出时点，而非提示下的二择一选择。
因此 **2B-G6 不能与 1A-G1 / 1B-G1 / 2A-G1 合并**，它只服务于 2B 内部的比较。

即便是可合并的三组，也来自不同招募批次与不同后测组合。合并统计时必须保留研究编号并检验研究间差异。
...
- 将 `feed--gray` 改为 CSS 变量控制饱和度，适配 30%、65% 两种操纵水平与 100% 不变的基线。
